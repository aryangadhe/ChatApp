...
import { AuthContext } from '../../constest/AuthContest';
...
const ProfilePage = () => {
  ...
  const [selecteedImg, setSelectedImg] = useState(null)
  ...
  const handleSubmit = async (e) => {
    ...
    reader.readAsDataURL(selecteedImg);
    reader.onload = async () => {
      ...
      rounded-lg'>
        <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1'>
          ...
          cursor-pointer'>
            ...
            <img src={selecteedImg ? URL.createObjectURL(selecteedImg) : assets.avatar_icon} alt="" 
            className={`w-12 h-12 ${selecteedImg && 'rounded-full'}`}/>
        ...
        <img className={`max-w-44 aspect-square rounded-full mx-10 max-sm:mt-10 
          ${selecteedImg && 'rounded-full'}`} src={authUser?.profilePic || assets.logo_icon} alt="" />
      </div>
    </div>
  )
}
